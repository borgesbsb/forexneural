...
void OnTick()
  {
   ...
   CopyTicks(_Symbol,ticks,COPY_TICKS_ALL);
...
//datetime time = ticks[0].time;
   ...
   for(int i=0; i<qtdecandlestend-1; i++)
     {
      var[i] = NormalizeDouble(((ticks[i].ask-ticks[1999].ask)/ticks[1999].ask*100),4);
      if(var[i]>percenttend/100)
         favoravel++;
     }
   double percentfavor = favoravel/qtdecandlestend;
   if(percentfavor>0.75)
      trade.Buy(volumeoper,_Symbol,tick.ask,tick.bid-pontosbesl*_Point,0,"BE COMPRA");

//      double var01 = NormalizeDouble(((ticks[0].ask-ticks[1999].ask)/ticks[1999].ask*100),4);
//      double var02 = NormalizeDouble(((ticks[1].ask-ticks[1999].ask)/ticks[1999].ask*100),4);
//      double var03 = NormalizeDouble(((ticks[2].ask-ticks[1999].ask)/ticks[1999].ask*100),4);
//      double var04 = NormalizeDouble(((ticks[3].ask-ticks[1999].ask)/ticks[1999].ask*100),4);
//      double var05 = NormalizeDouble(((ticks[4].ask-ticks[1999].ask)/ticks[1999].ask*100),4);

//Print("time: ",time," ticks01: ",var01," ticks02: ",var02," ticks03: ",var03," ticks04: ",var04," ticks05: ",var05);
  }
