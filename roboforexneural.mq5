            ...
            if(previsao_temp != 0.0)
              {
               if(previsao_temp > tick.ask && (!PossuiPosCompra() && !PossuiPosVenda()))
                 {
                  trade.Buy(volumeoper,_Symbol,tick.ask,0,previsao_temp,"COMPRA");
               ...
               if(previsao_temp < tick.bid && (!PossuiPosCompra() && !PossuiPosVenda()))
                 {
                  trade.Sell(volumeoper,_Symbol,tick.bid,0,previsao_temp,"VENDA");
      ...
      if(recebido2>tick.ask)
        {
         ...
         trade.Buy(volumeoper,_Symbol,tick.ask,candle[1].low,recebido2,"NEURAL COMPRA");
        }
      else
        {
         //Alert("recebido: ",recebido);
         trade.Sell(volumeoper,_Symbol,tick.bid,candle[1].high,recebido2,"NEURAL VENDA");
        }
     } */
  }
